...
interface LockedStakingInterface {
     ...
     * * CAN ONLY BE USED BY THE OWNER
     *
     * @param key the key of the config to be dropped
     */
    function dropStakingConfig(bytes32 key) external;
     ...
     * * dropped: when the config was dropped (dropStakingConfig())
     *      or inactive (flushOldConfig()).
    ...
    /**
     * returns True if the config is dropped
     * @param key the config encoded-unique key
     */
    function isDropped(bytes32 key) external view returns (bool);
     ...
     *          if the config is dropped or inactive, it only considers
