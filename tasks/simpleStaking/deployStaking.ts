...
task("deploySimpleStaking", "task to deploy the smart contract to manage locked staking.")
    .addPositionalParam(
        "deployerAddress", 
    ...
    .addOptionalParam("outputFile", "Path to the output config file. By default uses tasks/outputs/deploySimpleStaking/output_<datetime>.json", "")
    .setAction(async (taskArgs, hre) => {
        const now = new Date();
        const defaultOutputFolder = path.join(path.dirname(path.dirname(__dirname)), "outputs", "deploySimpleStaking")
        // check output file
        let outputFile;
